...
<form method="post" action="zapisz_dane.php">
	<table>
		...
		<tr>
			...
			<td><label>Należność [PLN]</label></td>
			<td><input type="number" name="naleznosc_kary" value=0 required></td>
		<tr>
		<tr>
			<td></td>
			<td><input type="submit" value="DODAJ"><input type="reset" value="WYCZYŚĆ"></td>
			<input type="text" name="strona" value="form_kary.php" hidden>
		</tr>
	</table>
</form>

<table><caption>Lista Kar</caption>
	<tr>
		<th>ID kary</th>
		<th>Klient</th>
		<th>Należność [PLN]</th>
...
if($pdo != null)
{
    $zapytanie = 'SELECT id_kary, klient_id_klient, k2.nazwisko ,k2.imie ,naleznosc_kary FROM wypozyczalnia.kary k1 inner join wypozyczalnia.klient k2 on k1.klient_id_klient = k2.id_klient';
    $wynik = $pdo->query($zapytanie);
    $rekordy = $wynik->fetchAll(PDO::FETCH_ASSOC);
    foreach ($rekordy as $rekord) {
        echo "<tr>";
        echo "<td>".$rekord['id_kary'] . "</td>";
        echo "<td>". $rekord['nazwisko'] ." ".$rekord['imie']."</td>";
        echo "<td>". $rekord['naleznosc_kary'] . "</td>";
        echo "</tr>";
    }
}
?>
